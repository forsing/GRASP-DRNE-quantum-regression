...
# Varijanta: DRNE – rekurzivna duboka mreža (višeslojna agregacija suseda)
 
"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
DRNE (Deep Recursive Network Embedding): rekurzivna agregacija susednih reprezentacija
u više slojeva; H^(l+1)[v] = (1+eps)*H^(l)[v] + agregat(H^(l)[u] : u in N(v)).
"""
...
DRNE_LAYERS = 3 # 3 sloja za DRNE   
DRNE_EPS = 0.0  # 0.0 epsilon za DRNE
...
def compute_drne_embeddings(M, k=EMBED_DIM):
    G = nx.Graph()
    for i in range(1, 40):
        G.add_node(i)
    for i in range(1, 40):
    ...
    n_comp = min(k, 39)
    H = vecs[:, :n_comp].astype(float)
    if H.shape[1] < k:
        H = np.hstack([H, np.tile(H[:, -1:], (1, k - H.shape[1]))])
    for _ in range(DRNE_LAYERS - 1):
        H_new = (1.0 + DRNE_EPS) * H.copy()
        for i in range(39):
            node = i + 1
            neigs = list(G.neighbors(node))
            if neigs:
                idx = [u - 1 for u in neigs]
                H_new[i] += H[idx].mean(axis=0)
        H = H_new
    for d in range(k):
        col = H[:, d]
...
def main():
    ...
    emb = compute_drne_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (DRNE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
...
Početne reprezentacije: 
prve k Laplacian sopstvenih vektora (39×k).

DRNE_LAYERS = 3: 
u svakom sloju: 
H_new[v] = (1 + eps)*H[v] + mean(H[u] za u in N(v)).

Na kraju se vrednosti normalizuju u [0, π].
"""
